...
# CONFIGURATION
# ═══════════════════════════════════════════════════════════════════
_dotenv_loaded = load_dotenv("settings.env")  # V14: capture whether a settings.env file was actually found
...
CFG = {
    ...
    "MAX_CONSEC_LOSS":   _i("MAX_CONSECUTIVE_LOSSES",   "6"),     # V14: 5→6 (graduated threshold now handles 2-5)
    ...
    "PAPER_MIN_TRADES":  _i("PAPER_MIN_TRADES",         "1"),      # V14: 2→1 per spec
...
def check_viable(direction: str, entry: float, sl: float,
    ...
    if direction == "BUY":
        gross  = ((tp1 - entry)*tp1_qty +
                  (tp2 - entry)*tp2_qty +
                  (tp3 - entry)*tp3_qty)
        # V14 FIX: R:R reward used to benchmark against TP2, a leftover from
        # when TP2 closed 35% of the position. Under V13/V14 (15% TP1 / 0%
        # TP2 / 85% runner-to-TP3), TP2 is not a real exit — TP3 is where
        # 85% of size actually books. Benchmarking R:R against TP3 reflects
        # the real reward the runner strategy is aiming for.
...
# ═══════════════════════════════════════════════════════════════════
# V14: EARLY REVERSAL DETECTION
# Two genuinely LEADING reversal signals (not lagging trend-confirmation
# indicators like EMA crosses). Both are scored as ordinary rules feeding
# into the same independent bull_score/bear_score — consistent with
# "highest independent score wins", not a separate override mechanism.
# When BOTH agree in the same direction (confluence >= 2), that confluence
# is also used to allow a controlled bypass of the 4H trend_aligned() hard
# block below — otherwise a genuine reversal would always be rejected
# until the slower 4H trend itself flips, defeating the point of catching
# it early.
# ═══════════════════════════════════════════════════════════════════
def _find_pivots(series: pd.Series, left: int = 3, right: int = 3,
                  mode: str = "low") -> List[Tuple[int, float]]:
    """Simple fractal pivot detector: a bar is a pivot low/high if it is
    the strict min/max within a window of `left`/`right` bars either side."""
    pivots = []
    n = len(series)
    for i in range(left, n - right):
        window = series.iloc[i - left: i + right + 1]
        center = series.iloc[i]
        if mode == "low"  and center == window.min() and (window == center).sum() == 1:
            pivots.append((i, float(center)))
        elif mode == "high" and center == window.max() and (window == center).sum() == 1:
            pivots.append((i, float(center)))
    return pivots


def detect_divergence(df: pd.DataFrame, lookback: int = 40) -> Tuple[Optional[str], int]:
    """RSI divergence on 15m — price and momentum disagreeing is one of the
    earliest objective reversal tells, well before a trend EMA flips.
    Bullish: price makes a LOWER low while RSI makes a HIGHER low (selling
    pressure fading before price confirms). Bearish: mirror image."""
    if len(df) < lookback + 10 or "rsi" not in df.columns:
        return None, 0
    recent = df.iloc[-lookback:]
    lows  = _find_pivots(recent["low"],  mode="low")
    highs = _find_pivots(recent["high"], mode="high")

    if len(lows) >= 2:
        (i1, p1), (i2, p2) = lows[-2], lows[-1]
        r1, r2 = float(recent["rsi"].iloc[i1]), float(recent["rsi"].iloc[i2])
        if p2 < p1 and r2 > r1:
            return "BULL", 15 if (r2 - r1) > 5 else 10

    if len(highs) >= 2:
        (i1, p1), (i2, p2) = highs[-2], highs[-1]
        r1, r2 = float(recent["rsi"].iloc[i1]), float(recent["rsi"].iloc[i2])
        if p2 > p1 and r2 < r1:
            return "BEAR", 15 if (r1 - r2) > 5 else 10

    return None, 0


def detect_structure_break(df: pd.DataFrame, trend_up: bool, trend_down: bool,
                            lookback: int = 40) -> Tuple[Optional[str], int]:
    """Break-of-structure — price closing beyond the most recent swing
    high/low AGAINST the prevailing trend, the classic price-action tell
    that a trend may be turning, ahead of any EMA/ADX confirmation."""
    if len(df) < lookback + 10:
        return None, 0
    recent = df.iloc[-lookback:-1]   # exclude the still-forming current bar
    price  = float(df["close"].iloc[-1])
    lows   = _find_pivots(recent["low"],  mode="low")
    highs  = _find_pivots(recent["high"], mode="high")

    # V14: volume-confirm the break. A structure break on thin volume is a
    # textbook false-breakout/trap; the same break with a volume expansion
    # is meaningfully more reliable. Full weight only when volume is at
    # least 1.3x its 20-period average; otherwise treat as a weaker,
    # unconfirmed break rather than discarding it entirely.
    vol_r = float(df["vol_r"].iloc[-1]) if "vol_r" in df.columns else 1.0
    weight = 12 if vol_r >= 1.3 else 6

    if trend_down and highs and price > highs[-1][1]:
        return "BULL", weight
    if trend_up and lows and price < lows[-1][1]:
        return "BEAR", weight

    return None, 0


def trend_aligned(tfs: dict, direction: str,
                   reversal_confluence: int = 0) -> Tuple[bool, str]:
    ...
    V14: a strong reversal confluence (RSI divergence AND structure break
    both agreeing with the signal direction — confluence >= 2) is allowed
    to bypass this block. A single reversal clue is not enough to fight
    the 4H trend, but two independent, agreeing leading signals are."""
    ...
    if direction == "BUY"   and t4d:
        blocked, reason = True, "BUY signal but 4H is bearish"
    elif direction == "SHORT" and t4u:
        blocked, reason = True, "SHORT signal but 4H is bullish"
    # Also block if 1H contradicts (belt-and-suspenders for strong trends)
    elif direction == "BUY"   and t1d and not t4u:
        blocked, reason = True, "BUY signal but 1H bearish and 4H not bullish"
    elif direction == "SHORT" and t1u and not t4d:
        blocked, reason = True, "SHORT signal but 1H bullish and 4H not bearish"

    if blocked and reversal_confluence >= 2:
        logger.info(f"Trend filter bypassed by reversal confluence "
                    f"({reversal_confluence} signals): {reason}")
        return True, ""
    if blocked:
        return False, reason
    return True, ""
...
def generate_signal(tfs: dict, sentiment: float, regime: Regime) -> Signal:
    ...
    # Rule 15: V14 NEW — RSI divergence (15m) — leading reversal signal
    div_dir, div_str = detect_divergence(df15)
    if div_dir == "BULL": bull.append(("RSI bullish divergence", div_str))
    elif div_dir == "BEAR": bear.append(("RSI bearish divergence", div_str))

    # Rule 16: V14 NEW — Break-of-structure (15m) — leading reversal signal
    bos_dir, bos_str = detect_structure_break(
        df15, bool(df15["trend_up"].iloc[-1]), bool(df15["trend_down"].iloc[-1]))
    if bos_dir == "BULL": bull.append(("Bullish structure break (BOS)", bos_str))
    elif bos_dir == "BEAR": bear.append(("Bearish structure break (BOS)", bos_str))

    bull_score = sum(w for _, w in bull)
    bear_score = sum(w for _, w in bear)

    # V14: cap the "single-event cluster" — a candle pattern, RSI divergence,
    # and structure break can all fire off the SAME pivot (three lenses on
    # one event). That's legitimate confluence, but uncapped it could let
    # one local candle outweigh several independent trend/timeframe rules
    # combined. Cap this specific cluster's combined contribution at 30 pts
    # per side; everything else in the score is untouched.
    pattern_bull = sum(p.strength for p in patterns if p.direction == "BULL")
    pattern_bear = sum(p.strength for p in patterns if p.direction == "BEAR")
    cluster_bull = pattern_bull + (div_str if div_dir == "BULL" else 0) + (bos_str if bos_dir == "BULL" else 0)
    cluster_bear = pattern_bear + (div_str if div_dir == "BEAR" else 0) + (bos_str if bos_dir == "BEAR" else 0)
    if cluster_bull > 30:
        bull_score -= (cluster_bull - 30)
        logger.info(f"Reversal cluster capped: BULL {cluster_bull} → 30")
    if cluster_bear > 30:
        bear_score -= (cluster_bear - 30)
        logger.info(f"Reversal cluster capped: BEAR {cluster_bear} → 30")

    TOTAL      = 157    # V14: 130 + 27 to account for new max reversal-rule weight (15+12)
    ...
    # V14 FIX: REMOVED score-flip override.
    # The old logic force-flipped direction to match the 4H trend whenever
    # scores were within 15% of each other — even when the OTHER side had
    # the objectively higher independent score. That silently defeated the
    # "trade whichever side scores highest" rule and, combined with the 4H
    # bonus above, produced a near-permanent SHORT bias during the extended
    # 4H downtrend (bot took shorts almost exclusively). BUY and SHORT are
    # now scored fully independently and the higher raw score always wins;
    # the 4H trend still contributes points (see bonus above) but can no
    # longer override the winner outright.
    if scores_close:
        logger.info(
            f"SCORE TIE (gap {score_gap} within 15% of {score_max}) — "
            f"keeping highest independent score: {direction}")

    # Trend conflict block (unchanged from V7, now with reversal-confluence bypass)
    # V14: count how many of the two leading reversal signals (divergence,
    # structure break) agree with the WINNING direction — this is what lets
    # a genuine early reversal override the stale 4H filter below.
    reversal_confluence = sum(1 for d in (div_dir, bos_dir) if d == ("BULL" if direction == "BUY" else "BEAR"))
    aligned, conflict_reason = trend_aligned(tfs, direction, reversal_confluence)
    if not aligned:
    ...
    # V14: Graduated confidence escalation (per spec) — required win-prob
    # rises with each consecutive loss instead of jumping straight to paper:
    # 2 losses → +5% | 3 → +10% | 4 → +15% | 5 → +20% | 6 → paper mode
    # (the 6-loss paper-mode switch itself is handled in _tick via
    # CFG["MAX_CONSEC_LOSS"], set to 6)
    if   consec_now >= 5: threshold = max(threshold, CFG["MIN_WIN_PROB"] + 20.0)
    elif consec_now == 4: threshold = max(threshold, CFG["MIN_WIN_PROB"] + 15.0)
    elif consec_now == 3: threshold = max(threshold, CFG["MIN_WIN_PROB"] + 10.0)
    elif consec_now == 2: threshold = max(threshold, CFG["MIN_WIN_PROB"] + 5.0)
...
def place_order_safe(direction: str, qty: float, leverage: int) -> Tuple[bool, float, float]:
    """V14: Final safety net for 'Margin is insufficient' (-2019).
    The exchange-balance sync in _tick() should prevent this in almost all
    cases, but balance can still shift between the sync read and the order
    fill (a funding charge lands, a stray fee posts, etc). Rather than
    dropping the trade entirely, retry up to twice with qty cut by 40%
    each time (never below the 0.001 BTC exchange minimum). Returns
    (success, fill_price, qty_actually_used) so the caller can record the
    real filled size."""
    attempt_qty = qty
    for attempt in range(3):
        ok, fp = place_order(direction, attempt_qty, leverage)
        if ok:
            if attempt_qty != qty:
                logger.warning(f"Order filled at reduced size {attempt_qty} BTC "
                                f"(originally {qty} BTC) after margin retry")
            return True, fp, attempt_qty
        # place_order already logged the error; only retry on margin-insufficient
        next_qty = round(attempt_qty * 0.6, 3)
        if next_qty < 0.001 or attempt == 2:
            break
        logger.warning(f"Retrying order with reduced qty {next_qty} BTC "
...
def get_available_balance() -> Optional[float]:
    """V14 FIX: Fetch REAL available USDT margin from Binance.
    Root-cause fix for repeated 'Margin is insufficient' failures — the bot
    was sizing every trade off the locally-tracked `capital` KV value, which
    can drift from the actual exchange wallet (funding payments, rounding,
    manual actions, etc). High-confidence signals (bigger risk_pct → bigger
    margin requirement) failed silently for hours while only small,
    low-confidence signals happened to fit. Returns None on API failure so
    callers can fall back to local capital instead of blocking entirely."""
    try:
        acct = get_client().futures_account()
        for a in acct.get("assets", []):
            if a.get("asset") == "USDT":
                return float(a.get("availableBalance", 0.0))
    except Exception as e:
        logger.warning(f"Could not fetch exchange balance, using local capital: {e}")
    return None
...
class TradingEngine:
    ...
    async def _tick(self):
        ...
        if daily_loss >= CFG["MAX_DAILY_LOSS"] and mode == "LIVE":
            ...
            tg(f"⚠️ Daily loss {daily_loss:.1f}% hit.\n"
               ...
               f"Auto-resumes after {CFG['PAPER_MIN_TRADES']} profitable "
               f"paper trades (after fees & tax) — no time gate.", "WARN")
            return
        ...
        # ── V14 FIX: SYNC TO REAL EXCHANGE BALANCE BEFORE SIZING ─
        # Prevents 'Margin is insufficient' failures caused by local capital
        # drifting from the actual Binance wallet. Only relevant for LIVE —
        # paper trades never touch the exchange.
        if not is_paper:
            exch_bal = get_available_balance()
            if exch_bal is not None and exch_bal > 0:
                if abs(exch_bal - capital) > max(0.50, capital * 0.02):
                    logger.warning(
                        f"CAPITAL DRIFT: local ${capital:.2f} vs exchange "
                        f"${exch_bal:.2f} — syncing to exchange balance")
                kset("capital", round(exch_bal, 4))
                capital = exch_bal
    ...
    async def _open_trade(self, signal, qty, capital,
        ...
        if not is_paper:
            ok, fp, filled_qty = place_order_safe(signal.direction, qty, signal.leverage)
            if not ok:
                return
            if filled_qty != qty:
                # V14: order filled at a reduced size after a margin retry —
                # use the real filled qty for everything downstream so the
                # DB, dashboard, and PnL math reflect what's actually open
                # on the exchange.
                qty = filled_qty
    ...
    async def _manage_trade(self, db, trade, price, tfs,
        ...
        if not trade.tp1_hit:
            ...
            if tp1_triggered:
                ...
                # V14 FIX: "breakeven" buffer was 0.05% (1.0005 / 0.9995),
                # but round-trip taker fees alone are ~0.08% (entry 0.04% +
                # exit 0.04%), before funding. That meant the runner's
                # "safety net" after TP1 was actually a guaranteed small
                # NET LOSS once fees were applied — a direct contributor to
                # "losses high, wins small". Buffer raised to 0.12% so the
                # worst case after TP1 is a true (small) net win, not a
                # fee-eaten loss.
                if trade.direction == "BUY":
                    be = trade.entry_price * 1.0012
                    if be > trade.stop_loss:
                        trade.stop_loss = round(be, 2)
                else:
                    be = trade.entry_price * 0.9988
                    if be < trade.stop_loss:
                        trade.stop_loss = round(be, 2)
                db.commit()
                return
        ...
        if trade.direction == "BUY":
            ...
            # V14 FIX: this block used to run TWICE — once with the adaptive
            # trail_mult (0.6/1.0/1.5x by trend strength), then again with a
            # hardcoded 0.8x that silently overrode it whenever trail_mult
            # was wider than 0.8 (i.e. every strong-trend trade, trail_mult
            # =1.5). Since almost all trades in your log are SHORT, this
            # meant the "let winners run in a strong trend" trail was being
            # clamped back down to a tight fixed 0.8x on nearly every trade
            # — a direct cause of runners getting stopped out early. BUY
            # never had this duplicate block, so SHORT and BUY now behave
            # symmetrically.
            if atr > 0:
                new_sl = price + trail_mult * atr
                if new_sl < trade.stop_loss:
    ...
    async def _close_trade(self, db, trade, exit_price,
        ...
        if not is_paper:
            ...
            # V14: per spec, a paper "win" that counts toward the recovery
            # gate must be positive after fees AND tax liability — not just
            # net_for_trading > 0 (which ignores the tax that would actually
            # be owed on the gain).
            if trade.status == "WIN" and total_net > pnl["tax_liability"]:
    ...
    async def _check_recovery(self):
        ...
        # V14: Per spec, recovery is trade-outcome-based, not time-gated —
        # switch back to LIVE as soon as PAPER_MIN_TRADES trades have shown
        # a genuine profit (after fees AND tax, see paper_wins counting in
        # _close_trade) rather than waiting out PAPER_HOURS as well.
        profit_ok = paper_pnl > 0
        trades_ok = paper_wins >= CFG["PAPER_MIN_TRADES"]

        # V14 FIX: pre-existing gap — recovery only checked daily conditions
        # via day_start_cap reset, never the WEEKLY loss limit. That let the
        # bot flip back to LIVE, then get hard-stopped by the weekly circuit
        # breaker one tick later (since paper P&L is simulated and never
        # touches real capital) — a pointless flip-flop that also forced
        # manual_stop=true, requiring you to manually intervene for no
        # reason. Now checks weekly loss before resuming live at all.
        capital_now = float(kget("capital", str(CFG["INITIAL_CAPITAL"])))
        week_cap    = float(kget("week_start_cap", str(capital_now)))
        week_loss   = (week_cap - capital_now) / week_cap * 100 if week_cap > 0 else 0.0
        week_ok     = week_loss < CFG["WEEKLY_LOSS_LIMIT"]

        if profit_ok and trades_ok and not week_ok:
            logger.warning(
                f"Paper trading profitable (${paper_pnl:.2f}, {paper_wins} wins) "
                f"but weekly loss {week_loss:.1f}% still >= "
                f"{CFG['WEEKLY_LOSS_LIMIT']}% limit — staying in PAPER "
                f"rather than resuming live only to hit the weekly stop.")
            return

        if profit_ok and trades_ok and week_ok:
            capital = capital_now
            kset("mode",               "LIVE")
            kset("stop_reason",        "")
            kset("paper_pnl",          "0.0")
            ...
            tg(f"✅ Paper trading profitable — {paper_wins} winning trades.\n"
...
function setOT(d) {
  ...
  el.innerHTML = `
    ...
    <div class="row"><span class="rl">TP1 15% (fee-lock) ${p1c}</span><span class="rv profit">${f2(t.tp1)}</span></div>
    <div class="row"><span class="rl">TP2 (reference only)</span><span class="rv profit">${f2(t.tp2)}</span></div>
    <div class="row"><span class="rl">TP3 85% (adaptive trail runner)</span><span class="rv profit">${f2(t.tp3)}</span></div>
...
async def main():
    ...
    logger.info(f"✓ Multi-TP   → TP1 {CFG['TP1_PCT']}% fee-lock + {100-CFG['TP1_PCT']-CFG['TP2_PCT']:.0f}% adaptive ATR runner")
    logger.info(f"✓ Paper gate → {CFG['PAPER_MIN_TRADES']} profitable trades required (no time gate)")
    # V14: Loudly flag if a Railway/env override has drifted the TP split
    # away from the intended V13/V14 spec (this is exactly what caused the
    # bot to actually run 40%/35%/25% while the code said 15%/0%/85%).
    # V14: settings.env silently overrides any Railway env var that ISN'T
    # explicitly set on Railway (python-dotenv only fills in unset keys by
    # default). This is almost certainly why TP1_CLOSE_PCT/TP2_CLOSE_PCT/
    # PAPER_MIN_TRADES kept showing old values even though Railway's own
    # Variables tab never had them — a settings.env file committed to the
    # repo has been quietly supplying them on every container start.
    if _dotenv_loaded:
        logger.warning(
            "⚠️ settings.env FILE DETECTED in the deployed repo — any keys "
            "in it override code defaults for variables NOT set in Railway. "
            "This is very likely the source of the TP1/TP2/PAPER_MIN_TRADES "
            "drift. Check/remove settings.env in the GitHub repo if its "
            "values are stale.")
    if CFG["TP1_PCT"] != 15.0 or CFG["TP2_PCT"] != 0.0:
        logger.warning(
            f"⚠️ TP SPLIT OVERRIDE DETECTED: TP1_CLOSE_PCT={CFG['TP1_PCT']}% "
            f"TP2_CLOSE_PCT={CFG['TP2_PCT']}% (spec is 15% / 0%). "
            f"Check Railway env vars TP1_CLOSE_PCT / TP2_CLOSE_PCT if this "
            f"is unintentional.")
    ...
    tg(f"🚀 *BTC Trading System v{VERSION} Started*\n"
       ...
       f"Paper gate: {CFG['PAPER_MIN_TRADES']} profitable trades (no time gate)\n"
