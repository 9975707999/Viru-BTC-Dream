...
def position_size(capital: float, entry: float,
    ...
    # Always enforce minimum order size
    if qty < MIN_QTY:
        qty = MIN_QTY
...
function setSt(d) {
  ...
  document.getElementById('cap-big').textContent = f2(d.capital);
  document.getElementById('cap-big').className   = 'big '+(d.capital>=d.initial?'profit':'loss');
